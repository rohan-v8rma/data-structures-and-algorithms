...
class MaxHeap {
    ...
    public:
        ...
        void deleteRoot() {
            ...
            while(true) {
                if( hasLeftChild(index) && (leftChild(index) > heap[largestIndex]) ) {
                    largestIndex = getLeftChildIndex(index);
                }

                // largestIndex can be 0, or it can be leftChildIndex resulting from the previous if-condition.
                if( hasRightChild(index) && (rightChild(index) > heap[largestIndex]) ) {
...
MaxHeap heapify(int* arr, int size) {
    MaxHeap newHeap;
